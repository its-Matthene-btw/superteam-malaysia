...
export default function SeedPage() {
  ...
  const sqlSchema = `-- Run this in your Supabase SQL Editor first:
...
-- 2. Storage Buckets (Run these manually in the Storage UI or via API)
-- Bucket names: "avatars", "logos"`;
  ...
  const handleSeed = async () => {
    if (!confirm('This will insert data into your Supabase database. Ensure you have run the SQL schema first. Continue?')) return;
    setLoading(true);
    try {
      ...
      if (data.errors.length === 0) {
        ...
        toast({ variant: "destructive", title: "Partial Success", description: "Some tables failed to seed." });
      }
    } catch (err: any) {
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto space-y-10 animate-fade-up">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Follow these steps to activate your dynamic CMS.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <Card className="glass border-white/10 flex flex-col">
            <CardHeader className="border-b border-white/5 bg-white/5">
              <CardTitle className="flex items-center gap-2 text-white uppercase tracking-widest text-xs">
                <Terminal className="w-4 h-4 text-primary" /> Step 1: SQL Schema
              </CardTitle>
            </CardHeader>
            <CardContent className="p-6 space-y-4 flex-1 flex flex-col">
              <p className="text-xs text-muted-foreground leading-relaxed">
                Copy this SQL and run it in your **Supabase SQL Editor** to create the tables.
              </p>
              <div className="relative flex-1 group">
                <pre className="p-4 rounded bg-black/50 border border-white/5 text-[10px] font-code text-primary h-[300px] overflow-y-auto whitespace-pre-wrap">
                  {sqlSchema}
                </pre>
                <Button 
                  size="sm" 
          ...
          <Card className="glass border-white/10 flex flex-col">
            ...
            <CardContent className="p-6 space-y-6 flex-1 flex flex-col">
              <p className="text-xs text-muted-foreground leading-relaxed">
                After creating the tables, click the button below to inject the initial ecosystem content.
              </p>
                  ...
                  {result.errors.length > 0 && (
                    <div className="p-4 rounded border border-destructive/20 bg-destructive/10 space-y-2">
                      <h4 className="font-bold text-[10px] uppercase tracking-widest text-destructive flex items-center gap-2">
                        <AlertCircle className="w-3 h-3" /> System Logs
                      </h4>
