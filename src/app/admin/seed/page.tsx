...
import { Database, CheckCircle2, Copy, ShieldCheck, Zap, AlertTriangle, Trash2 } from 'lucide-react';
...
`;
  ...
  const handleEcosystemSeed = async () => {
    ...
    try {
      ...
      toast({ title: "Ecosystem Wiped & Seeded", description: "Fresh ecosystem blueprints are now live." });
    } catch (err: any) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <Card className="lg:col-span-1 glass border-white/10 flex flex-col">
            ...
            <CardContent className="p-6 space-y-4 flex-1">
              <p className="text-xs text-muted-foreground">Run this in Supabase first to create the Ecosystem tables.</p>
          ...
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
            <Card className="glass border-white/10 flex flex-col">
              ...
              <CardContent className="p-6 space-y-6 flex-1 flex flex-col">
                <p className="text-xs text-muted-foreground">Non-destructive seed for members, events, stats, and testimonials.</p>
            ...
            <Card className="glass border-white/10 border-red-500/20 flex flex-col bg-red-500/5">
              <CardHeader className="border-b border-white/5 bg-white/5">
                <CardTitle className="flex items-center gap-2 text-red-400 uppercase tracking-widest text-xs">
                  <Trash2 className="w-4 h-4" /> 3. Ecosystem Wipe & Seed
                </CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-6 flex-1 flex flex-col">
                <p className="text-xs text-red-200/60 font-medium">DESTRUCTIVE: Deletes all Projects, Categories, and Features before re-seeding blueprints.</p>
                <div className="mt-auto">
