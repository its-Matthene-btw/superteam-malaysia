...
export default function CommunityCarousel() {
  ...
  return (
    <section className="w-full bg-black border-t border-white/10 pt-20">
      <div className="max-w-[1600px] mx-auto px-10 mb-10 border-x border-white/10 pt-10">
        <div className="flex flex-col gap-6 mb-16">
          <div className="inline-flex items-center gap-2 border border-white/10 px-4 py-1.5 rounded-full text-[10px] font-bold tracking-[2px] text-muted-foreground uppercase">
            <span className="text-primary text-base">✦</span> The Community
          </div>
          <h2 className="text-5xl lg:text-7xl font-headline font-bold uppercase tracking-tight leading-none max-w-3xl">
      ...
      <div className="w-full border-b border-white/10">
        <div 
          ref={sliderRef}
          className="max-w-[1600px] mx-auto border-t border-white/10 border-x border-white/10 overflow-x-hidden cursor-grab active:cursor-grabbing"
          ...
          <div className="flex whitespace-nowrap" ref={trackRef}>
            {displayMembers.map((member, idx) => (
              <div 
                key={`${member.id}-${idx}`}
                className={cn(
                  "relative flex flex-col min-w-[380px] h-[560px] border-r border-white/10 bg-[#050505] transition-all duration-300 group cursor-pointer overflow-hidden",
                  "hover:bg-primary",
              ...
              >
                ...
                <div className="flex-1 p-8 flex flex-col">
                  <span className="font-code text-[10px] uppercase tracking-[2px] text-muted-foreground mb-3 group-hover:text-black transition-colors">
                    [{member.track}]
                  </span>
                  <h3 className="text-3xl font-headline font-extrabold uppercase tracking-tighter mb-6 group-hover:text-black transition-colors">
                    {member.name}
                  </h3>
